...
| A6 | Unification des 3 idiomes temporels (datetime-local négociation / grille When2Meet / agenda drag&drop). Décomposé : **A6a** clarté de fuseau + écho lisible ✅ LIVRÉ · **A6b** picker de négociation basé calendrier (`ScrimSlotCalendarPicker`, drop-in, clic = sélectionne jusqu'à 5 créneaux dans une grille hebdo, chips récap ; sortie datetime-local inchangée ; ancien `ScrimSlotPicker` supprimé) ✅ LIVRÉ · **A6c** helpers date/fuseau partagés entre les 3 surfaces (⏳, DRY interne). | L | ◐ EN COURS |
