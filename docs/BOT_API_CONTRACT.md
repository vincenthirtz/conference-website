...
| `mvp.public.opened`               | Admin `POST /api/admin/matches/[matchId]/mvp-public` (action `open`, depuis le cockpit régie)                                                                                                                | `{ matchId, roundName, team1Name, team2Name, closesAt, candidates: [{ memberId, label, teamName }, ...] }` — **poussé, jamais sondé** : la fenêtre du scrutin public dure ~10 min, soit la période entière du poller de clôture MVP, qui le raterait. Un `open` rejoué sur un scrutin déjà ouvert n'émet rien (sinon double message). |
| `mvp.public.closed`               | Admin `POST /api/admin/matches/[matchId]/mvp-public` (action `close`, depuis le cockpit régie)                                                                                                               | `{ matchId, winnerLabel, winnerMemberId, reason, team1Name, team2Name, bySource: { twitch, discord } \| null }` — le bot édite son message SANS composants. Le bot ne peut ni ouvrir ni clore lui-même : deux autorités sur la même urne donneraient un scrutin fermé d'un côté et ouvert de l'autre. |
...
- **Noms des cartes dans le digest** — les compteurs seuls ne disent pas _quoi_
...
ce que `matches.lobby_code` fait pour la _jonction_ au lobby.
...
#### `GET | POST /api/bot/v1/teams/:teamId/invitations`

La capitaine invite une joueuse depuis Discord (`/inviter send`,
`/equipe inviter`). **Auth** : `x-api-key` + `actorDiscordUserId`, qui doit être
le `captain_id` de l'équipe (sinon `403`). **Rate-key** :
`bot-team-invitations`. **Idempotency** : oui sur le POST.

**POST — body**

| Clé                   | Requis | Note                                             |
| --------------------- | ------ | ------------------------------------------------ |
| `actorDiscordUserId`  | oui    | La capitaine.                                    |
| `targetDiscordUserId` | oui    | L'invitée. `400` si c'est l'appelante elle-même. |
| `role`                | non    | Rôle d'équipe proposé ; absent ⇒ `player`.       |
| `battleTag`           | non    | Pré-rempli sur l'invitation.                     |
| `comment`             | non    | Tronqué à 1000 caractères.                       |

**POST — 201** : `{ success: true, invitation, teamName }`. `invitation` est la
ligne `demandes` créée, `payload` compris.

**404 particulier** : « La joueuse ciblée n'a pas lié son compte Discord au
site ». L'invitée doit exister côté site — c'est ce qui rend la suite possible.

**PAS DE LIEN PARTAGEABLE, ET C'EST VOULU.** Contrairement au chemin site
(`POST /api/teams/invitations`), cette route ne pose pas de
`payload.invite_token_hash` et ne renvoie aucune `invite_url`. Le 404 ci-dessus
garantit que l'invitée a déjà un compte lié ; elle dispose donc de **trois**
voies d'acceptation qui ne demandent aucun lien : le DM à boutons envoyé par le
bot, la commande `/mes-invitations`, et son espace joueuse. Un lien n'ajouterait
aucune capacité — seulement un secret de 32 octets circulant sur Discord, où il
se colle en deux clics dans un salon public. Le POST étant de surcroît
idempotent, ce secret dormirait 5 min de plus dans `bot_idempotency`.

Conséquence à connaître côté site : ces invitations n'ont **ni email ni lien**.
L'espace équipe les signale « Envoyée sur Discord » (via `source`), et
`POST /api/teams/invitations/:id` sait les **prolonger** — la relance repousse
l'expiration et fabrique alors un lien, rendu à la capitaine _sur le site_.

**GET** — `?status=` (défaut `pending`, `all` accepté), `?type=`, `?limit=`.
Renvoie `{ invitations: [...] }` en camelCase (`createdAt`, `processedAt`…),
...
| [`tenants/[id]/plan-checkout.ts`](../pages/api/admin/tenants/[id]/plan-checkout.ts)                           | POST               | owner        | Lien de paiement HelloAsso. Un owner NON plateforme ne peut le generer que pour un espace de son perimetre (403 TENANT_OUT_OF_SCOPE sinon).                                                                                                            |
| [`tenants/[id]/export.ts`](../pages/api/admin/tenants/[id]/export.ts)                                         | POST               | owner        | Archive JSON de toutes les donnees d'un espace (manifeste `utils/tenants/tenantTables.ts`). Secrets et caches exclus par construction ; troncature signalee dans `report.truncated`.                                                                   |
| [`tenants/[id]/lifecycle.ts`](../pages/api/admin/tenants/[id]/lifecycle.ts)                                   | POST               | owner        | Etat d'un espace : active / suspended (402 + motif cote bot et API) / archived (404) / purge_scheduled. Motif >= 10 caracteres obligatoire hors reouverture. `conference` protege.                                                                     |
| [`tenants/[id]/bot-invite.ts`](../pages/api/admin/tenants/[id]/bot-invite.ts)                                 | GET                | owner        | Lien d'invitation du bot PORTEUR de l'espace (state signe). Mode `direct` si DISCORD_OAUTH_REDIRECT_URI est configuree, `manual` sinon.                                                                                                                |
| [`../onboard/discord-callback.ts`](../pages/api/onboard/discord-callback.ts)                                  | GET                | PUBLIC       | Retour Discord apres installation : rattache le serveur a l'espace porte par le state signe, puis redirige avec un drapeau lisible. Inerte sans state valide.                                                                                          |
| [`tenants/[id]/domain.ts`](../pages/api/admin/tenants/[id]/domain.ts)                                         | GET, POST          | caster/admin | Etat du domaine propre + enregistrements DNS a creer ; POST lance la verification (TXT de preuve, CNAME de routage). Seul un domaine `verified` est route.                                                                                             |
| [`tenants/[id]/invitations/index.ts`](../pages/api/admin/tenants/[id]/invitations/index.ts)                   | GET, POST          | caster/admin | Invitations d'un espace. POST cree un jeton (hache, 14 j) et envoie l'email avec le compte d'envoi DE L'ESPACE. Role invite <= role de l'invitant.                                                                                                     |
| [`tenants/[id]/invitations/[invitationId].ts`](../pages/api/admin/tenants/[id]/invitations/[invitationId].ts) | DELETE             | caster/admin | Revoque une invitation en vol. Idempotent (200 + `revoked:false` si rien a revoquer).                                                                                                                                                                  |
| [`../invitations/[token].ts`](../pages/api/invitations/[token].ts)                                            | GET, POST          | PUBLIC       | Lecture d'une invitation (etat, espace, role, indice d'adresse) et acceptation par l'utilisateur connecte : cree le compte staff s'il manque (role global `caster`) puis rattache via `tenant_staff`.                                                  |
...
| [`pages/api/auth/battlenet/callback.ts`](../pages/api/auth/battlenet/callback.ts) | GET     | session cookie Supabase joueur  | Query `code`, `state` (+ cookie `bn_oauth_state`). `302` vers `returnTo` avec `?battlenet=` : `verified` (BattleTag lié), `linked_no_match` (BattleTag déjà rattaché à un autre compte — conflit d'unicité), `already_linked` (identité déjà présente), `error` (state invalide, échange KO…). `302 /login` si la session Supabase est perdue. `503` si dormant. Endpoint de redirection : pas de JSON body. **Effet de bord TCG (2026-09-15)** : une fois le lien écrit, crédite `BATTLENET_VERIFIED_COINS` (100 pièces, sans paquet) au porte-monnaie du tenant de la requête, **une fois à vie** par joueuse et par compte Blizzard (`utils/tcg/grantBattlenetVerified.ts`, `source_kind = battlenet_verified`, migration `tcg_battlenet_verified.sql`), et émet `tcg.reward_granted` (`reason: 'battlenet_verified'`). Best-effort : un refus d'écriture ou une exception est journalisé et ne change **jamais** le statut `?battlenet=`. **Paramètre de retour additif** : `&tcg=battlenet_reward` est ajouté à la redirection **seulement** quand cet appel a crédité (ex. `/player/profile?battlenet=verified&tcg=battlenet_reward`) ; rejeu, déjà récompensée, refus ou exception → absent. L'espace joueuse affiche « +N pièces », N relu dans `battlenet-status` (`reward.coins`), jamais dans l'URL. Rate-limit **20 / min**. |
| [`pages/api/player/battlenet-status.ts`](../pages/api/player/battlenet-status.ts) | GET     | Bearer joueur (`withAuthRoute`) | `200 { configured:boolean, linked:boolean, battleTag:string\|null, verifiedAt:string\|null, reward:{ coins:number, claimable:boolean }\|null }`. **`reward` (ajout additif, 2026-09-15)** : la récompense TCG de la vérification, que la carte annonce sans écrire de nombre (`coins` lu dans `earnSources.ts`) ; `null` = non activée ; `claimable: false` = déjà reçue (tous tenants confondus) **ou** registre illisible — on ne promet pas à l'aveugle. `401` si Bearer absent/invalide. Rate-limit **30 / min**.                                                                                                                                                                                                                                                                              |
...
Le contenu du fichier est vérifié par ses _magic bytes_, pas seulement par le
...
| Route                                                                                                                               | Methods           | Auth                                                                | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`pages/api/player/tcg/collection.ts`](../pages/api/player/tcg/collection.ts)                                                       | GET               | Bearer joueur (`withAuthRoute`)                                     | `200 { cards[], distinct, total, pool?, nextCursor }`, les plus rares d'abord. **Pagination par curseur, rétrocompatible (2026-09-15)** : sans `limit` ni `cursor`, la collection entière comme avant avec `nextCursor: null` ; avec `?limit=1..200` (défaut 40 si seul `cursor` est fourni) et/ou `?cursor=<nextCursor>`, une page et `nextCursor` tant qu'il en reste. Ordre **total** : rareté décroissante puis clé de sujet `<kind>:<id>` — sur la rareté seule, les ex æquo n'ont pas d'ordre garanti et deux pages se chevaucheraient. On pagine l'**affichage**, pas la lecture : `distinct`/`total` restent ceux de la collection entière et seules les faces de la page sont relues. Curseur opaque, **sans aucune URL d'image** (une face se relit à chaque affichage). `400 { code:'invalid_limit' \| 'invalid_cursor' }` ; un curseur au-delà de la fin rend une page vide. Les cartes sont lues par tranches (`utils/tcg/readOwnedCards.ts`) : l'ancien `.limit(5000)` était en réalité plafonné à 1000 lignes par PostgREST et amputait en silence les collections de plus de 200 paquets. `Cache-Control: private, no-store`. **La collection se déduit, elle n'est pas stockée** : lecture des paquets ouverts puis de leurs cartes, comptage côté serveur — le socle a écarté une table `collection`, un agrégat finissant par diverger du détail qui le nourrit. Les faces sont **relues à chaque appel**, jamais figées au tirage : c'est ce qui rend le retrait de consentement rétroactif sur les cartes déjà distribuées, et une photo n'est servie que si elle est `approved` **et** non révoquée. Rareté rendue = la meilleure possédée (elle est figée par tirage et peut différer d'un exemplaire à l'autre). Trois sujets : `player`, `team` et `map` — une carte de map est **toujours `common`**, une map n'ayant pas de palmarès donc pas de prestige à mesurer (lui donner une rareté variable inventerait une seconde échelle de valeur à côté de celle des joueuses) ; elle se distingue par la brillance seule. Le dénominateur de progression ajoute la taille du registre de maps aux viviers lus en base. Chaque carte porte **`recyclable`** : le couple `{ packId, position }` qu'exige `POST /api/player/tcg/recycle` — et qui porte son idempotence via `source_ref = <pack_id>:<position>` — ou **`null` dès qu'il n'y a qu'un exemplaire**, la route refusant le dernier (`not_a_duplicate`). ⚠️ **L'exemplaire désigné est le MOINS précieux** (plus basse rareté, non brillant à rareté égale) : la route de recyclage accepte n'importe quel exemplaire pourvu qu'il en reste un autre et **ne regarde pas lequel part**, donc envoyer le premier venu détruirait un jour l'épique d'une joueuse en lui laissant la commune. La carte s'affiche avec sa **meilleure** rareté tandis qu'on propose la **pire** — les deux coexistent sans se contredire. Rate-limit **60 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [`pages/api/player/tcg/packs.ts`](../pages/api/player/tcg/packs.ts)                                                                 | GET, POST         | Bearer joueur (`withAuthRoute`)                                     | `GET 200 { packs[], unopened, balance, boosterPrice, earn:{ matchWin, scrimWin, welcomeGift, twitchDrop?, checkinStreak:{ length, coins, packs }, placement:[{ maxRank, coins, packs }], battlenetVerified:{ coins, packs }, collectionSet:{ coins, packs } }, nextCursor }` — **pagination par curseur, rétrocompatible (2026-09-15)** : sans paramètre, les 200 paquets les plus récents comme avant, plus `nextCursor` qui dit s'il en reste ; `?limit=1..200`, `?cursor=<nextCursor>`, `?status=unopened\|opened` (l'espace joueuse ne lit que les fermés : sans ce filtre, un paquet fermé plus ancien que la première page ne pouvait pas s'ouvrir). Ordre `(granted_at DESC, id DESC)`. Le curseur est interpolé dans un filtre PostgREST : validé en forme exacte (horodatage + UUID), sinon `400 invalid_cursor` ; `400 invalid_limit` / `invalid_status`. **`unopened` est désormais un COMPTE de tous les paquets fermés**, indépendant de la page — l'ancien calcul sur les 200 lignes lues sous-estimait. `Cache-Control: private, no-store`. La réponse du `POST` porte **`isNew`** par carte (sujet non possédé avant ce paquet, recyclées exclues), décidé par le serveur puisque la page ne voit plus toute la collection ; omis si la lecture échoue. (`twitchDrop` **seulement si le drop est réellement branché** — une chaîne connectée _et_ une récompense désignée ; sinon le barème annoncerait un gain que le webhook refuserait) — le prix **et** le barème viennent de l'API : l'interface les affiche sans les connaître, et sans `earn` la page montrait un solde et un prix sans jamais dire comment gagner des pièces. `POST { packId }` ouvre un paquet : **le tirage précède la consommation** — les cartes sont composées d'abord, le paquet n'est marqué ouvert qu'ensuite, donc un vivier vide ne coûte pas son paquet (`409 empty_pool`, paquet toujours fermé). ⚠️ **`empty_pool` est devenu quasi inatteignable** depuis les cartes de map : le vivier des maps est un registre en mémoire (`config/maps/overwatch.ts`), jamais vide, donc un tenant sans aucune joueuse ni équipe classée reçoit un paquet **complet de maps** plutôt qu'un refus — un paquet non vide valant mieux qu'un paquet refusé. La garde reste en place pour le jour où le registre serait vidé. Réservation atomique (`opened_at IS NULL`) : deux clics simultanés ne l'ouvrent pas deux fois (`409 already_opened`). Si l'écriture des cartes échoue après réservation, celle-ci est **relâchée** — mieux vaut un paquet fermé qu'un paquet ouvert et vide. La rareté n'est calculée que pour les 5 sujets tirés, jamais pour tout le vivier ; illisible, elle retombe sur `common` plutôt que de faire échouer l'ouverture. `404` (et non 403) sur un paquet qui n'est pas le sien. Composition : **trois joueuses, une équipe, une map** (`TEAM_SLOTS` + `MAP_SLOTS`, emplacements réservés et non probabilités par carte — un tirage indépendant sortirait un jour un paquet entier d'équipes). Les maps ne **s'ajoutent pas** au paquet, elles occupent une place : le comblement va aux joueuses d'abord, aux équipes ensuite, aux maps en dernier. La réponse du POST porte les **faces** des cartes tirées (`displayName`/`imageUrl` pour une joueuse, `name`/`slug`/`logoUrl` pour une équipe, `slug`/`name`/`imageUrl` pour une map — où le slug **est** l'identifiant, les maps vivant dans un registre et non dans une table), même forme que la collection à `count` près : sans elles la page ne pouvait rien montrer de l'ouverture et rechargeait la collection, où le tirage se fondait en silence. Elles sont lues par `readPlayerFaces`/`readTeamFaces`, donc la révélation hérite de la garantie de consentement au lieu d'ouvrir une seconde voie d'accès aux photos. Rate-limit **60 / min** (GET), **30 / min** (POST). |
| [`pages/api/player/tcg/booster.ts`](../pages/api/player/tcg/booster.ts)                                                             | POST              | Bearer joueur (`withAuthRoute`)                                     | Achète un booster avec ses pièces. Ne crée qu'un paquet **fermé** ; le tirage appartient à l'ouverture. **Une transaction SQL** depuis le 2026-09-15 (`tcg_purchase_booster`, migration `tcg_wallet_atomic_balance.sql`) : verrou de la ligne de porte-monnaie, solde relu par `SUM(amount)` sur le registre, paquet, écriture `booster_purchase` et cache — tout ou rien (l'ancien enchaînement en trois requêtes laissait un recalcul concurrent effacer le débit : 3 boosters pour le prix de 2). `400 { code:'insufficient_funds', balance, price }` (solde du registre), `409 { code:'balance_changed' }` (verrou non obtenu, réessayer est sûr), `503 { code:'purchase_unavailable' }` tant que la fonction SQL n'est pas déployée — l'achat est refusé plutôt que fait sans verrou. Un `500` ne laisse jamais un paquet gratuit. Rate-limit **20 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [`pages/api/webhooks/twitch/tcg-drop.ts`](../pages/api/webhooks/twitch/tcg-drop.ts)                                                 | POST              | **Signature HMAC** (`TWITCH_EVENTSUB_SECRET`)                       | Drop TCG en direct. **Premier récepteur EventSub webhook du dépôt** — l'existant est en transport _websocket_, piloté par le navigateur de la régie, donc rien ne recevait d'événement serveur. `bodyParser: false`, sans précédent ici : le HMAC couvre les **octets exacts** du corps. Signature vérifiée en temps constant **avant** tout parsing et tout accès base ; fail-closed 503 sans secret ; horodatage hors ±10 min rejeté. **Tout cas non traitable est acquitté en 200** — un non-2xx fait retenter Twitch puis _désactiver la souscription_, donc un compte non lié ne doit pas couper le canal. `source_ref` = le **direct**, pas l'échange : le registre `utils/tcg/earnSources.ts` impose « un drop par live et par personne », garantie portée par l'unicité du registre. **Les deux prérequis d'origine sont LEVÉS** (2026-09-13) : `user_twitch_links` existe et est alimentée par OAuth (`/api/auth/twitch/*` — `team_members.twitch` reste auto-déclaré et inutilisable comme identité), et le CHECK `source_kind` admet `twitch_drop`. **La demande est RÉSOLUE, pas laissée en attente** : `granted` → `FULFILLED` ; identité non rattachée, hors direct ou récompense non configurée → `CANCELED`, **ce qui rend les points** — sans quoi une spectatrice perdait 10 000 points sans recevoir de carte. `replayed` ne rembourse **rien** (la carte a déjà été donnée : rembourser offrirait les deux), une autre récompense n'est jamais touchée, une panne transitoire non plus. Exige `channel:manage:redemptions`. La résolution est **non bloquante** : elle ne transforme jamais une attribution réussie en 503, ce qui ferait retenter Twitch et pourrait distribuer deux fois. **Un paquet en plus des pièces** (2026-09-15) : origine `drop`, sans match, accordé par `utils/tcg/grantCoinsThenPacks.ts` **après** le `RETURNING` des pièces — un retry Twitch rend `replayed` sans pièces ni paquet ; un paquet refusé laisse le drop `granted` en pièces seules (`packGranted: false` dans la réponse, `pack: null` dans `tcg.drop_granted`), jamais un 503. Rate-limit **600 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [`pages/api/admin/tcg/overview.ts`](../pages/api/admin/tcg/overview.ts)                                                             | GET               | Staff (`manage_tcg`)                                          | `200 { packs, coins, cards, photos, topSubjects, generatedAt }` — l'état de l'économie TCG en un appel, agrégé **côté serveur** (aucune ligne de détail ne sort). **`null` n'est pas `0`** : `0` = mesuré et vide, `null` = pas mesurable — une lecture en échec dégrade SA clé et le reste part en 200, pour qu'une panne ne se lise pas comme un effondrement. Les cartes n'ayant pas de `tenant_id` (elles sont cadrées par leur paquet), les comptages passent par les identifiants des paquets du tenant. Les cartes **recyclées** sont exclues des comptages de cartes possédées et rendues à part. Sommes de pièces **bornées** (PostgREST ne fait pas `SUM()` sans RPC) : un plafond atteint lève `truncated`. Les faces des sujets les plus distribués passent par `readCardFaces`, donc le panneau staff n'affiche jamais une photo que le public n'a plus le droit de voir. **`coins.earnedBySource`** ventile `earned` par origine (`source_kind` brut), **crédits seulement** : `earned` seul répond « combien », jamais « d'où », et les **drops en direct n'apparaissaient nulle part ailleurs** puisqu'ils ne créent aucun paquet. Clés **non closes** — ce sont les origines réellement présentes, jamais une ligne à zéro — donc un consommateur doit savoir afficher une clé inconnue ; une origine illisible tombe sous `unknown` plutôt que d'être écartée, sinon l'écart se lirait comme une perte. `null` = registre non lisible (même convention que `earned`), `{}` = lu et sans crédit. Calculée dans la boucle qui somme déjà les écritures : **aucune requête supplémentaire**, et elle hérite du même `truncated`. `packs.bySource` porte aussi **`welcome`** (cadeau d'accueil), **`drop`** (drop Twitch), **`placement`** (palmarès de tournoi) et **`streak`** (série de check-ins), chacune **comptée** et jamais déduite de `granted` moins les autres — la convention `null` ≠ `0` interdit l'arithmétique entre compteurs. Rate-limit **30 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [`pages/api/player/tcg/recycle.ts`](../pages/api/player/tcg/recycle.ts)                                                             | POST              | Bearer joueur (`withAuthRoute`)                                     | `POST { packId, position }` → `200 { recycled, refund }`. Recycle un **doublon** contre des pièces (`RECYCLE_REFUND_COINS`, dérivé de `BOOSTER_PRICE_COINS`). **Jamais le dernier exemplaire** : on exige qu'il reste au moins une autre carte non recyclée du même sujet, sinon `409 { code:'not_a_duplicate' }` — sans ce contrôle, « recycler un doublon » deviendrait « détruire sa collection contre de la monnaie ». La carte est **marquée** (`recycled_at`), pas supprimée : c'est ce qui garde le crédit explicable dans l'historique. Deux garde-fous indépendants contre le double recyclage, tous deux au schéma : marquage atomique (`recycled_at IS NULL`) → `409 already_recycled`, et `UNIQUE (tenant_id, user_id, source_kind, source_ref)` avec `source_ref = <pack_id>:<position>`. On marque **avant** de créditer — une carte retirée sans crédit se répare, un crédit sans carte retirée serait de la monnaie créée à partir de rien ; si le crédit échoue, le marquage est relâché. `404` (et non 403) sur une carte qui n'est pas la sienne. Rate-limit **30 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [`pages/api/player/tcg/wallet.ts`](../pages/api/player/tcg/wallet.ts)                                                               | GET               | Bearer joueur (`withAuthRoute`)                                     | `200 { entries[], shownTotal, truncated }` — « d'où viennent mes pièces ? ». `tcg_wallets.balance` n'est qu'un **cache** ; le registre `tcg_wallet_entries` est la source de vérité, et la migration portait déjà un index `(tenant_id, user_id, created_at DESC)` créé pour cette lecture — elle n'avait jamais été livrée. **Bornée à 50, pas paginée** : au rythme d'un mouvement par victoire, 50 lignes couvrent une saison ; `truncated` dit franchement que l'histoire continue. `shownTotal` est la somme des lignes **rendues**, pas du registre entier — annoncer un total qui ne correspond pas à ce qu'on affiche créerait un écart inexplicable. Chaque ligne porte `note` : le **motif d'une correction de l'équipe** (`admin_grant`), `null` pour toute autre source. L'API rend `sourceKind` **brut** (`match_win`, `scrim_win`, `booster_purchase`, `admin_grant`, `card_recycled`, `twitch_drop`, `welcome_gift`, `supporter_welcome`, `checkin_streak`, `tournament_placement`, `battlenet_verified`, `collection_set`), jamais un libellé : traduire au serveur l'obligerait à connaître la langue de la lectrice. Rate-limit **60 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| [`pages/api/admin/tcg/welcome-gift.ts`](../pages/api/admin/tcg/welcome-gift.ts)                                                     | GET, POST         | Staff (`manage_tcg`)                                          | Le **cadeau d'accueil** d'une édition : `GET` simule (`eligible`, `alreadyGifted`, `teams`, `reward`) sans rien écrire, `POST` distribue et rend les compteurs réels. **Deux méthodes plutôt qu'un bouton** : créditer d'un coup toutes les participantes ne se reprend pas — un paquet ouvert ne se rend pas — donc on annonce le nombre avant de laisser cliquer. **L'édition n'est pas un paramètre** : le serveur la résout (`resolveCurrentTournamentId`), sinon une URL forgée créditerait une édition archivée. Les participantes sont les rosters des **équipes engagées** (`stage_teams`), pas les feuilles de match : vérifié en base, la Cup 2026 a 30 matchs planifiés et **zéro** ligne de participation avant son début, donc un calcul sur les feuilles n'aurait crédité personne. Remplaçantes incluses, comptes dédoublonnés (une joueuse peut figurer dans deux équipes). **Rejouable** : l'unicité `(tenant_id, user_id, source_kind, source_ref = tournoi)` fait que relancer ne crédite que les nouvelles arrivées. ⚠️ **Ordre d'écriture volontaire** — pièces d'abord en `ON CONFLICT DO NOTHING … RETURNING`, paquet ensuite aux seuls comptes rendus : `tcg_packs` n'a aucune unicité exploitable ici (son index porte sur `source_match_id`, nul pour un cadeau, et deux NULL sont distincts en Postgres), donc l'ordre inverse multiplierait les paquets à chaque relance. Une écriture partielle ne lève pas. Le `POST` honore **`Idempotency-Key`** (`withAdminIdempotency`, clé `tcg-welcome-gift`) : double-clic, retry navigateur ou coupure réseau partielle ne distribuent pas deux fois. Ce n'est pas décoratif — le schéma protège des doublons de _pièces_ (unicité sur `source_ref`) mais **pas** des doublons de _paquets_, et cette couche ferme la fenêtre côté transport. Seules les réponses 2xx sont mises en cache, donc un 500 transitoire reste rejouable. Journalisé `tcg_welcome_gift_grant`. `private, no-store`. Rate-limit **20 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| [`pages/api/admin/tcg/battlenet-backfill.ts`](../pages/api/admin/tcg/battlenet-backfill.ts) | GET, POST | Staff (`manage_tcg`) | **Rattrapage** de la récompense « compte Battle.net vérifié » pour les comptes liés avant qu'elle existe. `GET` simule sans écrire : `{ eligible, alreadyRewarded, wouldGrant, discordDms, outsideSpace, ready, reward: { coins } }` ; `POST` passe **chaque** lien de l'audience par `grantBattlenetVerifiedReward` (l'écrivain du callback : mêmes index anti-abus, même annonce) et rend `{ eligible, granted, already, errors, reward }`. **Audience = l'espace du staff** : comptes liés **et** rattachés au tenant par une ligne `team_members` ou un `tcg_wallets` — l'index « une fois par personne » étant global, créditer ici une joueuse qui ne joue qu'ailleurs y consommerait sa récompense unique ; les liens écartés sont comptés (`outsideSpace`). `alreadyRewarded` = par personne **ou** par compte Blizzard, tous tenants confondus (affichage ; l'écrivain tranche). `discordDms` = DM `tcg.reward_granted` qui partiront (`null` = non mesurable). Lecture paginée (1000). **Une audience illisible n'est pas vide** : `500 AUDIENCE_UNREADABLE`, et le `POST` n'écrit alors rien. Relance sûre (ne crédite que les nouveaux). Journalisé `tcg_battlenet_backfill`. `POST` honore `Idempotency-Key` (clé `tcg-battlenet-backfill`). `private, no-store`. Rate-limit **20 / min**. |
| [`pages/api/admin/tcg/grant.ts`](../pages/api/admin/tcg/grant.ts)                                                                   | POST              | Staff (`manage_tcg`)                                          | `POST { userId, amount, reason, idempotencyKey }` → `200 { ok: true, entryId, balance, replayed }`. **L'écrivain d'`admin_grant`** : corriger à la main le solde de pièces d'une joueuse (`amount` entier non nul, valeur absolue ≤ 10 000, négatif = retrait ; `reason` 3 à 500 caractères après trim). **Une correction, pas une vente** : aucun moyen de paiement, aucun don — la monnaie se gagne. **Le registre d'abord** (`tcg_wallet_entries`, `source_ref = idempotencyKey`), puis `refreshBalance` ; `balance` = somme du registre plafonnée à zéro. **Idempotence portée par la base, sans migration** : l'unicité `(tenant_id, user_id, source_kind, source_ref)` interdit la seconde écriture ; rejouer la même clé rend l'écriture existante avec `replayed: true`, sans double crédit ni second journal. Une clé déjà appliquée à une **autre** joueuse ou un **autre** montant → `400 INVALID_BODY` (répondre « déjà fait » masquerait une correction jamais appliquée). **Un retrait ne rend jamais un solde négatif** : depuis le 2026-09-15 il passe par la fonction SQL `tcg_admin_debit` (verrou du porte-monnaie, `SUM(amount)` du registre, écriture et cache en une transaction) ; fonction absente → `503 WITHDRAWAL_UNAVAILABLE` (un crédit reste possible). **Une joueuse de l'espace seulement** (2026-09-15) : `userId` doit être rattaché au tenant — roster du tenant ou gain réel au registre du tenant, `admin_grant` exclu (`utils/tcg/tenantAttachment.ts`) — sinon `404 USER_NOT_FOUND`, indiscernable d'un compte inexistant et rendu avant toute lecture GoTrue (un owner tiers ne peut plus créer chez lui le porte-monnaie qui détournait la récompense Battle.net unique). Erreurs : `400 INVALID_BODY`, `404 USER_NOT_FOUND`, `409 INSUFFICIENT_BALANCE` (porte `balance`), rare `409 BALANCE_CHANGED` (verrou non obtenu ; rejouer la même clé est sûr), `503 WITHDRAWAL_UNAVAILABLE`. **Une erreur n'est pas une absence** : lecture de compte en échec → 500, jamais 404 ; insertion en erreur relue par la clé avant de conclure (504 après commit). **Le motif est visible par la joueuse** : recopié dans `tcg_wallet_entries.note` (migration `tcg_wallet_entries_note.sql`) et rendu par `GET /api/player/tcg/wallet` pour cette seule source ; aussi journalisé `tcg_admin_grant`, payload `{ userId, amount, reason, entryId }`. `private, no-store`. Rate-limit **30 / min**. |
| [`pages/api/admin/tcg/players.ts`](../pages/api/admin/tcg/players.ts) | GET | Staff (`manage_tcg`) | `GET ?q=` (2 à 100 caractères) → `200 { players: [{ id, email: null, display_name, battle_tag, team_name }] }` (20 au plus). Trouver le compte d'une joueuse pour **corriger son solde** sans exiger `manage_staff` (`/api/admin/users/search`). **Cantonnée à l'espace** (2026-09-15) : RPC `admin_search_tcg_players(p_tenant_id, p_query)` (migration `tcg_admin_search_players_scoped.sql`), filtrée EN BASE sur les comptes rattachés au tenant (roster ou gain réel) — l'ancienne RPC globale `admin_search_users` livrait email, BattleTag et équipe de toute la plateforme à n'importe quel owner, espace développeur compris. Recherche par pseudo et BattleTag ; `email` n'est ni cherché ni rendu (toujours `null`) ; `team_name` = une équipe de l'espace. Recherche en échec → 500, jamais une liste vide ; fonction absente → `503 SEARCH_UNAVAILABLE`, sans repli global. `private, no-store`. Rate-limit **60 / min**. |
| [`pages/api/admin/tcg/photos.ts`](../pages/api/admin/tcg/photos.ts)                                                                 | GET, PATCH        | Staff (`manage_tcg`)                                          | `GET 200 { photos: [{ userId, displayName, email, photoPath, photoUrl, submittedAt }], total }` — la file de relecture (`pending`, la plus ancienne d'abord, 100 au plus). `displayName` (`display_name`, sinon `full_name`) et `email` sont résolus en **un** appel RPC `admin_get_user_profiles` (`fetchAdminUserProfiles` ; il n'existe pas de table `profiles`). **L'enrichissement ne fait jamais échouer la file** : RPC en échec ou compte introuvable → `null` pour les deux, la file reste servie — `null` = « nom inconnu », jamais « compte supprimé ». `PATCH { userId, decision: approve\|reject, photoPath, reason? }` → `200 { status }` ; **`photoPath` obligatoire** (2026-09-15) : le chemin AFFICHÉ, rendu par le GET — l'écriture est conditionnelle à `photo_status = 'pending' AND photo_path = photoPath`, sans quoi une photo remplacée après l'affichage était approuvée sans avoir été vue, et un refus concurrent laissait la nouvelle orpheline dans le bucket public ; un refus **supprime du bucket le fichier confirmé par l'écriture** ; `409 NOT_PENDING` si la joueuse a retiré sa photo pendant la relecture (sa décision prime), `409 PHOTO_CHANGED` si elle l'a remplacée — rien n'est écrit ni supprimé, le panneau rafraîchit. Journalisé `tcg_photo_approve` / `tcg_photo_reject`. Rate-limit 60 / min (GET), 30 / min (PATCH). |
| [`pages/api/player/tcg/welcome-gift.ts`](../pages/api/player/tcg/welcome-gift.ts)                                                   | GET, POST         | Bearer joueur (**`withSubjectRoute`**)                              | `200 { gift: { coins, receivedAt } \| null }` — alimente la carte du tableau de bord, qui se retire quand il n'y a rien à dire. **L'absence de cadeau est un état normal**, pas une erreur : `gift: null`, jamais un `404`. Route distincte de `wallet` parce que celui-ci rend les 50 derniers mouvements — y chercher une ligne `welcome_gift` obligerait l'interface à connaître le vocabulaire du registre et à espérer que le cadeau soit encore dans la fenêtre. ⚠️ **Seule route `tcg/` à honorer `?as=`** : le tableau de bord est partagé avec la vue d'inspection staff, et une route lisant `user.id` y afficherait le cadeau de l'admin sur la page de la joueuse inspectée — les autres routes `tcg/` ont ce défaut. Le wrapper force `private, no-store` en inspection. **`POST` depuis le 2026-09-14** : réclamer le cadeau d'accueil **supportrice** (`grantSelfWelcome`), une fois par COMPTE — `source_ref` porte le tenant, pas le tournoi, ce qui le distingue du cadeau d'édition. Refusé à qui n'a pas le rôle de compte `supporter` (`not_eligible`) ou figure sur un roster (`on_roster`, sinon elle cumulerait les deux accueils : le rôle n'est qu'une étiquette choisie à l'inscription). ⚠️ **`allowActAs` n'est PAS activé** : `?as=` reste donc refusé hors GET, un staff ne peut pas réclamer à la place de quelqu'un, et un cadeau réclamé ne se rend pas. `packGranted` est **rendu à l'appelant** au lieu d'être avalé — c'est l'écart qu'un écran a masqué le 2026-09-14, laissant 58 comptes crédités sans paquet. Le `GET` rend en plus `welcomeClaimable`, calculé par le MÊME code en `dryRun`. Rate-limit **60 / min** en GET, **6 / min** en POST.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [`pages/api/player/tcg/photo.ts`](../pages/api/player/tcg/photo.ts)                                                                 | GET, POST, DELETE | Bearer joueur (`withAuthRoute`)                                     | `GET 200 { status, photoUrl, optedIn, rejectedReason }` — la joueuse voit sa propre photo même `pending`/`rejected`, le filtrage public appartient au lecteur des cartes. `POST { data, mimeType }` (base64, 2 Mio, PNG/JPEG/WebP) → `200 { status:'pending' }` ; `400 { code }` parmi `missing_data`, `unsupported_type`, `invalid_base64`, `too_large`, `content_mismatch`. `DELETE` → `200 { status:'revoked' }`. **Une lecture en échec n'est pas une absence** (2026-09-15) : `POST` rend `500` avant tout envoi si l'ancienne photo est illisible ; `DELETE` rend `500` sans rien écrire, et son effacement est conditionnel au chemin lu (un dépôt intercalé est relu, `409 PHOTO_CHANGED` après 3 courses) — sinon le fichier restait dans le bucket public sans aucune ligne pour le désigner. Rate-limit **5 / min** (POST), **10 / min** (DELETE).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| [`pages/api/player/tcg/sets.ts`](../pages/api/player/tcg/sets.ts) | GET | Bearer joueur (`withAuthRoute`) | `200 { sets[], rewardCoins, newlyRewarded[] }` — les **séries** (collections à compléter), **dérivées** des données existantes : maps d'un mode (`map_mode`), équipes engagées dans une édition (`tournament_teams`, `stage_teams`), roster d'une équipe pour une édition (`team_roster` : `match_participants` si `completed`, sinon `team_members` hors encadrement). 10 tournois visibles au plus, 3 cartes minimum par série. Chaque carte d'une série appartient au **vivier du tirage** (`utils/tcg/readDrawPool.ts`, partagé avec l'ouverture) : une série n'est jamais incomplétable, et le consentement photo n'y change rien. Chaque série : `{ key, kind, mode, tournamentId, tournamentName, teamId, teamName, total, owned, complete, missingNamed[{ kind: team\|map, id, name }], missingPlayers, rewarded, justRewarded }` — **les joueuses manquantes ne sont jamais nommées** (`missingPlayers` est un nombre). **Lecture qui peut écrire** : chaque appel récompense les séries complètes non encore récompensées (`collection_set`, `COLLECTION_SET_COINS`, sans paquet), idempotent par la clé du registre ; émet `tcg.set_completed` à la seule écriture réelle. Recycler après la récompense ne la reprend pas, recompléter ne la reverse pas. `500 { code:'sets_unreadable' }` sur lecture partielle — rien n'est écrit. `POST /api/player/tcg/packs` rend en ajout rétrocompatible `setsCompleted[]` (même forme que `newlyRewarded`). Pas de `?as=`. `private, no-store`. Rate-limit **30 / min**. |
| [`pages/api/player/tcg/showcase.ts`](../pages/api/player/tcg/showcase.ts) | GET, PUT | Bearer joueur (`withAuthRoute`) | La **vitrine** (opt-in) : jusqu'à 3 cartes exposées sur la fiche publique `/player/[userId]`. `GET 200 { enabled, cards[], unavailable, maxCards, publicProfileUrl }` — **désactivée par défaut** (`enabled: false` sans réglage). `PUT { enabled, cards: string[] }` (0 à 3 clés `player:<uuid>`, `team:<uuid>`, `map:<slug>`) → même forme ; `400 { code:'invalid_body' \| 'invalid_card' }`, `409 { code:'not_owned' }` (activer exige de posséder chaque carte ; **désactiver n'exige rien**, un retrait n'est jamais bloqué). Table `tcg_showcases` : **références de sujet seulement**, jamais d'image ni d'exemplaire ; les cartes sont **relues contre la possession réelle** à chaque affichage (une carte recyclée ou échangée en sort) et leurs faces passent par `readCardFaces` (filtre de consentement). Le `PUT` régénère la fiche (`revalidatePlayerCard`), qui régénère aussi les vitrines actives exposant la carte d'une joueuse dont la photo change. `private, no-store`. Rate-limit **60 / min** (GET), **20 / min** (PUT). |
| [`pages/api/player/tcg/trades/index.ts`](../pages/api/player/tcg/trades/index.ts) | GET, POST | Bearer joueur (`withAuthRoute`) | **Échanges de cartes.** `GET ?box=received\|sent&state=open\|closed&limit&cursor` → `200 { trades[], nextCursor }` (expiration paresseuse avant lecture, faces relues, `ownedCopies`/`tradeableCopies` de l'appelante sur les cartes demandées d'une proposition reçue en attente, rien de la collection de l'autre). `POST { recipientId, offered: Subject[1..5], requested: Subject[1..5] }` (corps **strict** : ni pièces, ni paquet, ni message ; **parité**) → `201 { trade: { id, expiresAt } }` via la fonction SQL `tcg_propose_trade` (consentements, ancienneté compte 14 j + collection 7 j, plafonds **par compte** en base : 5 envoyées / 10 reçues en attente, une par paire, 24 h après refus, cartes de paquets `victory`/`placement`/`purchase`/`trade` seulement). Codes : `invalid_body`, `invalid_items`, `self_trade`, `trading_disabled`, `collection_too_recent`, `recipient_unavailable`, `recipient_inbox_full`, `too_many_pending`, `already_pending`, `recently_declined`, `offered_not_owned`, `requested_not_available`. Émet `tcg.trade_proposed`. 60/min GET, 10/min POST. |
| [`pages/api/player/tcg/trades/[tradeId].ts`](../pages/api/player/tcg/trades/[tradeId].ts) | POST | Bearer joueur (`withAuthRoute`) | `POST { action: 'accept' \| 'decline' \| 'cancel' }` → `200 { trade: { id, status }, replayed }`. `accept` = fonction SQL `tcg_accept_trade` (**une transaction** : verrous consultatifs + verrous de ligne, possession **revérifiée**, 3 échanges acceptés / 24 h par compte, déplacement des lignes vers deux paquets `trade`, **aucune pièce**) — **idempotent** (`already_accepted` → `replayed: true`, pas de seconde annonce). `decline`/`cancel` = écriture conditionnelle. 404 hors de la paire ou du tenant. Codes : `not_found`, `not_pending`, `expired`, `stale`, `requested_unavailable`, `daily_limit` (429), `partner_daily_limit`, `not_eligible`. Émet `tcg.trade_resolved`. 30/min. |
| [`pages/api/player/tcg/trades/settings.ts`](../pages/api/player/tcg/trades/settings.ts) | GET, PUT | Bearer joueur (`withAuthRoute`) | `GET 200 { acceptsProposals, eligible, eligibleAt, eligibilityReason, limits, pending }` — **opt-in** (pas de ligne = `false`). `PUT { acceptsProposals }` : activer exige l'éligibilité (`409 collection_too_recent`) ; désactiver annule les reçues en attente (système, annoncées) et retire les envoyées (sans annonce). 60/min GET, 10/min PUT. |
| [`pages/api/player/tcg/trades/partners.ts`](../pages/api/player/tcg/trades/partners.ts) | GET | Bearer joueur (`withAuthRoute`) | `200 { partners: [{ userId, displayName }] }` — les volontaires **du tenant** seulement, servis à une appelante volontaire (`403 trading_disabled` sinon). Aucun email, aucune RPC de recherche plateforme. 30/min. |
| [`pages/api/player/tcg/trades/cards.ts`](../pages/api/player/tcg/trades/cards.ts) | GET | Bearer joueur (`withAuthRoute`) | Sans `userId` : mes cartes avec `copies`, `tradeableCopies`, `available`. `?userId=` : les **doubles échangeables** d'une partenaire volontaire (jamais sa collection ; `404 partner_not_found` sinon). Faces relues. 60/min. |
| [`pages/api/cron/tcg-trades-expire.ts`](../pages/api/cron/tcg-trades-expire.ts) | GET, POST | `CRON_SECRET` (Bearer ou `?secret=`) | Cron horaire (`netlify/functions/tcg-trades-expire-cron.ts`, `17 * * * *`) : passe en `expired` les propositions échues (tous tenants) et émet `tcg.trade_resolved` (`expired`), une fois. `200 { expired, duration_ms }`. |
| [`pages/api/bot/v1/players/by-discord/[discordUserId]/twitch.ts`](../pages/api/bot/v1/players/by-discord/[discordUserId]/twitch.ts) | GET               | Bot (`withBotRoute`, x-api-key)                                     | `200 { authUserId, discordUserId, configured, linked, twitchLogin, linkedAt, ctaUrl }` — alimente la commande **`/twitch`**. `ctaUrl` est composé **par le site**, en absolu : une base recopiée côté bot mentirait au premier changement. Il vise `/player/profile` et **non** la route OAuth — un lien reçu en DM s'ouvre souvent dans un navigateur déconnecté, où la page redirige proprement vers la connexion tandis que `/api/auth/twitch/start` rendrait un 401 JSON, c'est-à-dire un cul-de-sac. **Aucun `twitch_user_id` rendu**, seulement le pseudo : déjà public, et il sert à confirmer _quel_ compte est relié (une joueuse peut en avoir plusieurs). `404 NOT_LINKED` si le compte Discord n'est relié à aucun compte du site. Rate-limit **60 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| [`pages/api/player/twitch-status.ts`](../pages/api/player/twitch-status.ts)                                                         | GET, DELETE       | Bearer joueur (`withAuthRoute`)                                     | `GET 200 { configured, linked, twitchLogin, linkedAt }`. **Le pont qui rend le drop Twitch exploitable** : un événement EventSub livre un identifiant Twitch, jamais un `auth.users.id` — sans ce lien, le webhook répond `identity_not_linked` et personne ne reçoit rien. L'identité doit être **prouvée par OAuth** : `team_members.twitch` est auto-déclaré et ne convient pas, s'en servir laisserait n'importe qui encaisser les drops d'une autre. `twitchLogin` est un pseudo d'**affichage** (Twitch autorise le renommage) ; c'est `twitch_user_id`, jamais rendu, qui fait foi. `DELETE` retire le lien immédiatement et sans condition — les cartes déjà gagnées restent acquises. `configured: false` = fonctionnalité dormante, l'écran masque le bouton. Rate-limit **30 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| [`pages/api/auth/twitch/start.ts`](../pages/api/auth/twitch/start.ts) · [`callback.ts`](../pages/api/auth/twitch/callback.ts)       | GET               | **Public** (session cookie sur `start`, state signé sur `callback`) | Flux OAuth de rattachement. **AUCUN scope demandé** : on veut savoir qui vient, pas agir en son nom — le consentement se réduit à la lecture du nom d'utilisateur, et aucun jeton n'est conservé. Flux **distinct** de `/api/admin/twitch/connect` (chaîne, douze scopes d'écriture, jetons chiffrés), avec sa propre URL de retour `TWITCH_PLAYER_REDIRECT_URI` pour qu'un code destiné à l'un n'atterrisse pas dans l'autre. Le callback exige **trois** vérifications : state signé non périmé, cookie nonce concordant, **et** session égale au compte qui a lancé le flux — sans la dernière, un state volé rattacherait un compte Twitch au compte d'autrui depuis n'importe quel navigateur. Compte déjà lié ailleurs → `?twitch=already_linked` (on ne vole pas un lien). Rate-limit **10 / min** (start), **20 / min** (callback).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [`pages/api/admin/tcg/overlay-token.ts`](../pages/api/admin/tcg/overlay-token.ts)                                                   | GET, POST, DELETE | Staff (`manage_tcg`)                                          | Le lien de la **source navigateur OBS** du TCG. `200 { url, createdAt, lastUsedAt }`, `url: null` si aucun lien actif. **Un seul jeton actif par espace** (index partiel) : `POST` émet ET révoque le précédent — c'est le geste « le lien a circulé », et un overlay déjà configuré dans OBS devient muet jusqu'à ce qu'on y colle le nouveau. Journalisé **sans le jeton** : un journal se relit, un secret ne s'y met pas. `private, no-store`. Rate-limit **30 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [`pages/api/overlay/tcg/[token].ts`](../pages/api/overlay/tcg/[token].ts)                                                           | GET               | **Public**, porté par le jeton                                      | `200 { items[], theme }` — les annonces des 15 dernières minutes (20 au plus), pour une source navigateur OBS. **Public par nécessité** (une source navigateur ne se connecte pas), **borné par conception** : chaque élément ne porte qu'un `kind` (`twitch_drop`, `match_win`, `scrim_win`), un horodatage et un **pseudo Twitch** — déjà public dans le chat. Jamais un nom de compte du site, jamais une photo : une joueuse a pu choisir son nom pour un espace connecté sans vouloir le voir en surimpression d'un direct. La règle est tenue par `utils/tcg/overlayFeed.ts`, seul chemin par lequel un nom sort. Sans compte Twitch lié, `twitchLogin: null` et l'overlay affiche un libellé neutre. Jeton inconnu **ou** révoqué → `404` identique (distinguer apprendrait lesquels ont existé). `s-maxage=5` + `X-Robots-Tag: noindex`. Rate-limit **120 / min** (une source interroge en boucle des heures durant).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [`pages/api/admin/tcg/overlay-theme.ts`](../pages/api/admin/tcg/overlay-theme.ts)                                                   | GET, PUT          | Staff (`manage_tcg`)                                          | L'**habillage** de la source navigateur OBS. `200 { theme, lineMax, positions }` — `theme` est toujours COMPLET : une colonne nulle en base veut dire « garde le défaut du code », pas « vide », donc un espace qui n'a jamais ouvert l'éditeur obtient l'overlay d'origine à l'identique. Table **séparée** des jetons (`tcg_overlay_themes`) parce que le jeton **tourne** — régénérer révoque et insère — et emporterait l'habillage avec lui, précisément au moment le plus tendu. `PUT` est un **patch partiel** : seules les clés présentes sont écrites, et `null` est une valeur utile (vider une formulation restaure la phrase traduite, `media: null` retire l'illustration). Les deux formulations sont du **contenu, pas de l'i18n** — le texte de cette chaîne, dans sa langue — rendu en TEXTE, jamais en HTML. Média dans un bucket **public** : validation par **magic bytes** (PNG/JPEG/WebP 2 Mio, MP4/WebM 8 Mio), l'ancien fichier supprimé **après** que le nouveau est référencé. Le thème voyage avec les annonces sur la route publique, mais **ne transite pas** par `overlayFeed.ts` : ce module est la frontière de confidentialité, l'y faire passer diluerait sa seule raison d'être. `400 { code }` : `invalid_color`, `invalid_position`, `unsupported_type`, `too_large`, `content_mismatch`. `private, no-store`. Rate-limit **30 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| [`pages/api/player/hero-preferences.ts`](../pages/api/player/hero-preferences.ts)                                                   | GET, PUT          | Bearer joueur (`withAuthRoute`)                                     | `GET 200 { picks[], bans[], slots }` — trois héros préférés, trois évités. Listes **globales au compte** (clavées sur l'utilisateur seul, comme les liens Discord) : conservées en changeant d'équipe. N'avoir rien choisi est l'état normal → deux listes vides, jamais un `404`. `slots` vient de l'API pour que l'interface n'ait pas à recopier le nombre. `PUT { picks, bans }` **remplace les deux listes d'un bloc** (jamais un patch partiel : elles sont liées, et une écriture partielle laisserait passer un état contradictoire entre deux appels) ; `400 { code:'INVALID_BODY', fields }`. Le schéma borne à trois (`cardinality`) et interdit l'intersection (`NOT (picks && bans)`) ; il ne peut PAS garantir l'unicité intra-liste (Postgres refuse toute sous-requête dans un CHECK) ni l'existence d'un héros — **ces deux règles ne tiennent que par cette route**, contre le référentiel unique `utils/heroes/overwatch.ts`. La réponse est **relue en base** après écriture, jamais recopiée depuis la requête. Consommé par `readPlayerFaces` pour choisir le héros qui représente une joueuse sans photo. Rate-limit **60 / min**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

**Écrivains TCG hors des routes `tcg/`** (2026-09-15). Deux routes existantes
créditent désormais le TCG en effet de bord, sans changer leur contrat sauf
un champ additif :

- `POST /api/admin/tournament/[id]/finalize` — **palmarès** : pièces et paquets
  `placement` selon `PLACEMENT_TIERS` aux **titulaires** d'une équipe du top 8
  (`match_participants` du tournoi, hors remplaçantes), via
  `utils/tcg/grantPlacementRewards.ts`. Une récompense par personne et par
  tournoi : un rejeu à l'identique ne recrédite personne (et sert de reprise),
  un `force: true` ne repaie pas la différence de rang et ne reprend rien.
  Compte rendu additif dans la réponse : `tcg_placement_rewards`
  `{ status, eligible, granted, packsExpected, packsGranted }`.
- `POST /api/checkin/[token]` et `POST /api/bot/v1/matches/[matchId]/checkin`
  (entonnoir `redeemCheckinToken`) — **série de check-ins** : un check-in neuf
  qui clôt 5 check-ins consécutifs de l'équipe dans le tournoi crédite
  `CHECKIN_STREAK_COINS` et un paquet `streak` aux titulaires du roster
  (`utils/tcg/grantCheckinStreak.ts`, clé `<tournoi>:<match>`). Best-effort,
  réponse inchangée.

Aucun événement bot n'accompagne ces deux voies.
...
| [`pages/api/admin/tournament/[id]/auto-schedule.ts`](../pages/api/admin/tournament/[id]/auto-schedule.ts)               | POST    | Session staff, permission `manage_tournaments` | Respecte désormais les **contraintes de disponibilité** des équipes : le scheduler savait quand une équipe est _libre_, jamais quand elle a le _droit_ de jouer. `dryRun: true` calcule et renvoie le planning **sans rien écrire** (avec `constraintCount` et les conflits) ; `ignoreTeamConstraints: true` rétablit l'ancien comportement pour comparer. Le défaut reste l'écriture, pour ne pas casser les appels existants.                                                                                                                                                                                                |
