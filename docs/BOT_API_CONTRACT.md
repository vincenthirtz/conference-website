...
| [`pages/api/player/tcg/packs.ts`](../pages/api/player/tcg/packs.ts) | GET, POST | Bearer joueur (`withAuthRoute`) | `GET 200 { packs[], unopened, balance }`. `POST { packId }` ouvre un paquet : **le tirage précède la consommation** — les cartes sont composées d'abord, le paquet n'est marqué ouvert qu'ensuite, donc un vivier vide ne coûte pas son paquet (`409 empty_pool`, paquet toujours fermé). Réservation atomique (`opened_at IS NULL`) : deux clics simultanés ne l'ouvrent pas deux fois (`409 already_opened`). Si l'écriture des cartes échoue après réservation, celle-ci est **relâchée** — mieux vaut un paquet fermé qu'un paquet ouvert et vide. La rareté n'est calculée que pour les 5 sujets tirés, jamais pour tout le vivier ; illisible, elle retombe sur `common` plutôt que de faire échouer l'ouverture. `404` (et non 403) sur un paquet qui n'est pas le sien. Rate-limit **60 / min** (GET), **30 / min** (POST). |
