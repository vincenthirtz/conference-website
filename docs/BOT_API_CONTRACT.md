...
| `tcg.reward_granted`              | `utils/tcg/announceReward.ts` (`announceTcgRewards`) — série de check-ins (`utils/tcg/grantCheckinStreak.ts`) et palmarès de fin de tournoi (`utils/tcg/grantPlacementRewards.ts`) | `{ userId, discordUserId, discordUsername, reason: 'checkin_streak' \| 'tournament_placement', coins, packs, tournamentId, tournamentName, rank, streak, sourceRef, ctaUrl }` — **UN event PAR joueuse créditée**, émis sur les seules lignes que l'insertion a rendues (jamais sur un rejeu : finalisation relancée, check-in rejoué). `packs` = paquets **réellement** créés (0 si refusés). `rank` renseigné pour le palmarès, `streak` pour la série (sinon `null`). `sourceRef` = la référence du registre, clé de dédoublonnage côté bot. Hors `WEB_PUSH_EVENT_TYPES`. Consommé par `services/discord-bot/tcg-events.js` (DM). |
