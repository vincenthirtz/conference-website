...
- **`min_players` — coachs exclus** : les deux chemins (`demandes/register-team`, `teams/create-with-member`) comptent désormais les joueurs (player+substitute) hors coach. Les deux tables de capacité (`tournament_teams` vs `stage_teams`) sont **volontairement** distinctes (concepts différents) — non fusionnées.
  > **Amendé le 2026-08-27** — régression introduite le 2026-06-28 par le modèle invite-accept (`f3f99c3`) : `create-with-member` comptait `min_players` sur `insertedMembers`, or depuis ce modèle seule la CRÉATRICE y figure (les coéquipières sont invitées). Toute inscription automatique échouait donc en silence dès `min_players ≥ 2` — et en mode manager dès `min_players ≥ 1`, la créatrice ne jouant pas. Correctif : deux effectifs distincts. Roster **confirmé** ≥ `min_players` → inscription directe (inchangé) ; roster **déclaré** (confirmé + invitations émises) ≥ `min_players` → **candidature** `demandes` type `team_registration` en attente de validation staff. On n'inscrit PAS sur le déclaré : une invitation n'est pas une joueuse, et un roster entièrement en attente consommerait une place du bracket. Corollaire côté équipe : `GET /api/demandes/register-team` renvoie désormais `status` (`TeamRegistrationStatus`) et l'espace équipe porte la carte « Inscription au tournoi » — jusque-là `demandes/register-team` n'avait **aucun appelant côté front**, et le message « réessaie depuis ton espace capitaine » du wizard ne menait nulle part.
  >
  > **Assoupli le 2026-08-27, même jour** — `min_players` ne conditionne plus le DROIT DE CANDIDATER. Une équipe se compose souvent après s'être manifestée ; exiger le roster complet la rendait invisible du staff pendant toute sa constitution (constat sur l'édition 2026). Depuis : `POST /api/demandes/register-team` accepte quel que soit l'effectif et pose `payload.roster_players` / `payload.min_players` pour que le staff arbitre en connaissance de cause ; `create-with-member` dépose une candidature dès que l'inscription directe est hors de portée, sans seuil. `min_players` reste **inchangé en base (5)** et garde toutes ses autres fonctions : inscription automatique DIRECTE, `requiredStarters` de la santé d'équipe, relances Discord (`utils/teamMessages.ts`), audience email « roster incomplet », `teamsBelowMin` du dashboard. Le seul verrou dur restant sur la candidature est `max_teams` — candidater ne libère pas de place.
