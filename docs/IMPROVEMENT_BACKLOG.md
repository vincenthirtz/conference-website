...
| Q036 | 2026-09-16 | robustesse | haute | M | `utils/billing/helloassoAccount.ts`, `utils/helloasso.ts`, `pages/api/helloasso/{webhook,prize-checkout}.ts`, `pages/api/admin/{helloasso/credentials,tournaments/[id]/prize-pool}.ts` | done | **La cagnotte d'un tournoi tiers arrivait sur le compte de l'association**, sans reversement possible. Corrigé le 2026-09-16 : chaque espace relie SON compte HelloAsso (secrets chiffrés par tenant, écran « Réglages › Encaissement », identifiants vérifiés auprès de HelloAsso avant enregistrement) ; sans compte relié, ouvrir une cagnotte rend `409 HELLOASSO_NOT_CONNECTED`, le paiement est refusé et la page publique masque le bouton (`fundingReady`). Le paiement porte le nom du tournoi (back-office de l'association). Le webhook accepte désormais un jeton DÉRIVÉ par espace (`?tenant=<slug>&token=…`) — un secret partagé aurait permis à une association de déclarer des paiements pour une autre — et n'applique une contribution que si la cagnotte appartient à l'espace authentifié. L'espace de la Coupe continue d'encaisser sur le compte de l'association. 20 tests (`tests/unit/helloassoTenantCollect.test.ts`). |
