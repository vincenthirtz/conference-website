// utils/og/matchPoster.tsx
//
// L'affiche d'un match, dessinée une fois et servie par DEUX routes :
//   /api/og/match/<uuid>        → 1200×630, carte sociale (og:image)
//   /api/og/match/<uuid>/story  → 1080×1920, story Instagram / TikTok
//
// POURQUOI DEUX CHEMINS ET PAS UN `?format=story`. C'est la leçon d'une mise en
// production : sur Netlify, la clé de cache CDN d'une route Next ne varie que
// sur `__nextDataReq` et `_rsc` (en-tête `netlify-vary`). Tous les autres
// paramètres de query sont ignorés — la première variante mise en cache est
// donc servie à toutes les autres, et `?format=story` renvoyait fidèlement la
// carte 1200×630, y compris avec un paramètre anti-cache. Un format qui change
// la réponse doit changer l'URL, pas sa query.
//
...
/**
 * Rend l'affiche et l'écrit dans la réponse. `story` choisit le format.
 *
 * Ne lève jamais : les deux niveaux de repli (carte générique, puis réponse
 * vide) existent parce qu'un crawler social qui reçoit une erreur garde un
 * aperçu cassé en cache bien plus longtemps que notre incident.
 */
export async function renderMatchPoster(
  req: NextApiRequest,
  res: NextApiResponse,
  story: boolean
) {
  ...
  // CDN avec revalidation en tâche de fond.
  res.setHeader(
    'Cache-Control',
