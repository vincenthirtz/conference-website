...
import teamByIdHandler from '../../pages/api/teams/[teamId]';
...
describe('GET /api/teams/[teamId]', () => {
  ...
  it('returns the team when found', async () => {
    ...
    const req = makeReq({ query: { teamId: validUuid } });
    const res = makeRes();
  ...
  it('rejects an invalid UUID with 400', async () => {
    const req = makeReq({ query: { teamId: 'not-a-uuid' } });
    const res = makeRes();
    await teamByIdHandler(req, res);
  ...
  it('rejects an array id with 400', async () => {
    const req = makeReq({ query: { teamId: [validUuid, validUuid] } });
    const res = makeRes();
    await teamByIdHandler(req, res);
    expect(res.statusCode).toBe(400);
  });
  ...
  it('rejects non-GET methods with 405', async () => {
    const req = makeReq({ method: 'POST', query: { teamId: validUuid } });
    const res = makeRes();
    await teamByIdHandler(req, res);
    expect(res.statusCode).toBe(405);
  });
