...
// Palette & color utils
const TEAM_PALETTE = ['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#14b8a6','#f97316','#22c55e']
function hashCode(str: string){ let h=0; for(let i=0;i<str.length;i++){ h=(h<<5)-h+str.charCodeAt(i); h|=0 } return Math.abs(h) }
function colorForName(name:string){ return TEAM_PALETTE[ hashCode(name) % TEAM_PALETTE.length ] }
function teamColor(team: Team){ return team.color && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(team.color) ? team.color : colorForName(team.name) }
...
export default function Tournoi() {
  ...
  function buildRoundRobin(teams: Team[]): [Team, Team][][] {
    ...
    for (let r = 0; r < R; r++) {
      ...
      const right = rotating.slice(Math.floor((n - 1) / 2)).slice().reverse()
  ...
  useEffect(() => {
    ...
    built = built.map((m, i) => {
      ...
      return { ...m, date: d.toISOString(), timeLabel: `${pad(d.getHours())}:${pad(d.getMinutes())}` }
    })
    ...
    try {
      ...
      if (Array.isArray(dict)) {
        built = built.map(m => {
          ...
          return row ? { ...m, result: { home: Number(row.home)||0, away: Number(row.away)||0 } } : m
        })
      } else {
        built = built.map(m => {
          const v = (dict as Record<string, { home:number; away:number }>)[m.id]
          return v ? { ...m, result: { home: Number(v.home)||0, away: Number(v.away)||0 } } : m
        })
      }
    } catch {}
  ...
  const standings = useMemo(() => {
    const table = new Map<string, { team: Team; mp: number; mw: number; ml: number; mapsW: number; mapsL: number }>()
    teams.forEach(t => table.set(t.name, { team: t, mp: 0, mw: 0, ml: 0, mapsW: 0, mapsL: 0 }))
    for (const m of matches) {
      if (!isCountable(m)) continue
      const a = table.get(m.home.name)!
      const b = table.get(m.away.name)!
      a.mp++; b.mp++
      a.mapsW += m.result!.home; a.mapsL += m.result!.away
      b.mapsW += m.result!.away; b.mapsL += m.result!.home
      if (m.result!.home > m.result!.away) { a.mw++; b.ml++ } else { b.mw++; a.ml++ }
    }
  ...
  function MapsChip({ maps }: { maps: string }) {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-white/10 border border-white/15 font-mono text-xs md:text-sm tabular-nums">{maps}</span>
    )
  }
  ...
  function DiffChip({ diff }: { diff: number }) {
    ...
    const base = 'inline-flex items-center px-2 py-0.5 rounded-full font-mono text-xs md:text-sm tabular-nums border'
    const cls = neutral
    ...
    return <span className={cls}>{sign}{Math.abs(diff)}</span>
  }
  ...
  function TeamBadge({ team }: { team: Team }) {
    ...
    return (
      <div className="flex items-center gap-2 min-w-0">
        {team.img ? (
          <img src={team.img} className="w-7 h-7 rounded border-2" style={{ borderColor: color }} alt={team.name} />
        ) : (
          <div className="w-7 h-7 rounded flex items-center justify-center text-xs font-bold text-black border-2" style={{ backgroundColor: color, borderColor: color }} aria-hidden>
            {initials}
          </div>
        )}
        <span className="truncate" style={{ color }}>{team.name}</span>
      </div>
    )
  }
    ...
    return (
      <div className="p-4 rounded-2xl bg-white/5 border border-white/10 shadow-sm backdrop-blur">
        <div className="flex items-center justify-between mb-2">
          <div className="text-xs uppercase tracking-wider text-gray-200">{m.id} · BO{m.bo}</div>
          <div className="text-sm font-medium text-white">{formatDateHuman(m.date)}</div>
        </div>
    ...
    <div className="container mx-auto px-4">
      ...
      <section className="mt-8">
        <div className="p-5 rounded-2xl bg-white/5 border border-white/10">
          <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-white mb-4">Classement (Round Robin)</h2>
            ...
            <table className="min-w-full text-base md:text-[17px]">
              <thead className="text-left text-gray-200 uppercase tracking-wider text-[11px] md:text-xs">
                <tr className="text-gray-300">
                  ...
                  <th className="py-2"><abbr title="Matchs joués">MJ</abbr></th>
                  <th className="py-2"><abbr title="Victoires">V</abbr></th>
                  <th className="py-2"><abbr title="Défaites">D</abbr></th>
                  <th className="py-2"><abbr title="Maps gagnées-perdues">Maps</abbr></th>
                  <th className="py-2"><abbr title="Différence de maps">Diff</abbr></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/10">
                {standings.map((r, idx) => (
                  <tr key={r.name} className="hover:bg-white/5 transition">
                    <td className="py-2 text-gray-300 tabular-nums">{idx + 1}</td>
                    <td className="py-2"><TeamBadge team={teams.find(t => t.name === r.name) ?? { name: r.name }} /></td>
                    <td className="py-2 tabular-nums text-gray-200">{r.mp}</td>
                    <td className="py-2 font-semibold tabular-nums text-gray-100">{r.w}</td>
                    <td className="py-2 tabular-nums text-gray-300">{r.l}</td>
                    <td className="py-2"><MapsChip maps={r.maps} /></td>
                    <td className="py-2"><DiffChip diff={r.diff} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-gray-400 mt-3">Tiebreakers: Victoires &gt; Différence de maps &gt; Maps gagnées.</p>
        </div>
      </section>

      {/* Matchs */}
      <section className="mt-10">
        <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-white mb-4">Calendrier – Phase de poules (BO3)</h2>
      ...
      <section className="mt-10 mb-20">
        <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-white mb-2">Finale (BO5)</h2>
        {!finalMatch ? (
          <p className="text-gray-300">En attente des 6 résultats de poules…</p>
        ) : (
          <div className="space-y-3">
            <div className="text-gray-300">{formatDateHuman(finalMatch.date)}</div>
            <div className="p-4 rounded-2xl bg-yellow-500/10 border border-yellow-500/20">
              <div className="text-sm uppercase tracking-wider text-yellow-300 mb-2">BO5 – Premier à 3</div>
              <div className="flex items-center justify-between gap-4">
                <TeamBadge team={finalMatch.home} />
                <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-white/10 border border-white/15 font-mono text-sm tabular-nums">
                  {isCountable(finalMatch) ? `${finalMatch.result!.home}–${finalMatch.result!.away}` : '—'}
                </span>
            ...
            {champion && (
              <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
                <div className="text-emerald-300 font-semibold">🏆 Champion: {champion}</div>
              </div>
            )}
          </div>
        )}
