        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 pt-20 pb-12">
          ...
          <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-400">
                {tenants
                  ? `${tenants.length} tenant${tenants.length > 1 ? 's' : ''} configuré${tenants.length > 1 ? 's' : ''}`
                  : 'Chargement…'}
              </p>
            </div>
          ...
          {pending.length > 0 && (
            <div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200 flex items-center justify-between gap-3">
              <span>
                <strong>{pending.length}</strong> serveur{pending.length > 1 ? 's' : ''} Discord en attente d&apos;attribution.
              </span>
              <Link
          ...
          <section className="bg-neutral-800/50 backdrop-blur border border-neutral-700/50 rounded-2xl overflow-hidden">
            {tenants === null ? (
              <div className="py-16">
                <LoadingSpinner label="Chargement des tenants…" />
              </div>
            ) : visible.length === 0 ? (
              <EmptyState
                ...
                description={
                  tenants.length === 0
                    ? 'Aucun tenant configuré pour le moment.'
