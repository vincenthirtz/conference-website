...
export default async function handler(
  ...
  if (
    applyRateLimit(req, res, { max: 60, windowMs: 60_000 }, 'bot-match-checkin')
  )
    return;

  if (!process.env.BOT_API_KEY) {
    logger.error('[bot/matches/checkin] BOT_API_KEY is unset');
  ...
  if (match.is_bye) {
    return res.status(400).json({ error: 'Match marque bye, check-in inutile' });
  }
