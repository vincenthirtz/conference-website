// GET /api/bot/players/by-discord/[discordUserId]/stats
//
...
import { verifyBotApiKey } from '@/utils/botAuth';
...
export default async function handler(
  ...
  if (
    applyRateLimit(req, res, { max: 60, windowMs: 60_000 }, 'bot-player-stats')
  )
    return;
  if (!process.env.BOT_API_KEY) {
    logger.error('[bot/player/stats] BOT_API_KEY is unset');
  ...
  const currentMember =
    memberRows?.find((m) => m.team_id) ?? null; // au plus un (cf. next-match.ts)
