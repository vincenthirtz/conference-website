...
function emptyDiscordConfig() {
  return {
    ...
    mvp_results_channel_id: null as string | null,
...
async function handler(req: BotCrossTenantRequest, res: NextApiResponse) {
  ...
  const { data: configRow, error: configErr } = await supabaseAdmin!
    ...
    .select(
      'staff_log_channel_id, matches_live_channel_id, disputes_forum_channel_id, news_ingest_channel_id, scrims_announce_channel_id, free_players_channel_id, team_openings_channel_id, mvp_results_channel_id, captain_role_id, substitute_role_id, staff_role_owner_id, staff_role_admin_id, staff_role_caster_id, teams_voice_category_id, disputes_forum_tag_open_id, disputes_forum_tag_pending_id, disputes_forum_tag_resolved_id, extras, placement_roles'
    )
    .eq('guild_id', guildId)
    .maybeSingle();
