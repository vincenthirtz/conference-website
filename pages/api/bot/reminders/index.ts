...
import { getDiscordLinksForUsers, type DiscordLink } from '@/utils/discordLinks';
...
export default async function handler(
  ...
  if (applyRateLimit(req, res, { max: 60, windowMs: 60_000 }, 'bot-reminders'))
    return;

  if (!process.env.BOT_API_KEY) {
    logger.error('[bot/reminders] BOT_API_KEY is unset');
...
async function collectMatchCheckinReminders(): Promise<MatchCheckinReminder[]> {
  ...
  const { data: matches, error } = await supabaseAdmin!
    ...
    .eq('status', 'pending')
    .neq('is_bye', true)
    .or('team1_captain_dm_30_sent_at.is.null,team2_captain_dm_30_sent_at.is.null');
