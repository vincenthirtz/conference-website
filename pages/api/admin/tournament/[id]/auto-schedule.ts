...
async function handler(
  req: NextApiRequest,
  res: NextApiResponse<AutoScheduleResponse | { error: string; detail?: string }>,
  ctx: any
) {
  const { id } = req.query;
  ...
  try {
    ...
    if (mErr) {
      console.error(
        "auto-schedule: fetch matches error",
        mErr
      );
    ...
    const config: AutoSchedulerConfig = {
      ...
      estimatedDurationsMinutes:
        body.estimatedDurationsMinutes ?? {},
      resourceGapMinutes:
        typeof body.resourceGapMinutes === "number"
          ? body.resourceGapMinutes
          : 5,
      teamRestMinutes:
        typeof body.teamRestMinutes === "number"
          ? body.teamRestMinutes
          : 15,
      defaultResourceId:
        body.defaultResourceId ?? "default",
    };
      ...
      try {
        await logStaffAction({
          ...
          payload: {
            ...
            unscheduled_count:
              result.unscheduledMatchIds.length,
            scheduled_match_ids:
              result.scheduled.map(
                (s) => s.matchId
              ),
            unscheduled_match_ids:
              result.unscheduledMatchIds,
          },
        });
      } catch (e) {
        console.error(
          "auto-schedule: logStaffAction error",
          e
        );
      }
    }
    ...
    console.error(
      "[/api/admin/tournament/[id]/auto-schedule] error:",
      err
    );
    return res.status(500).json({
