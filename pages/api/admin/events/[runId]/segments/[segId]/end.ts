// pages/api/admin/events/[runId]/segments/[segId]/end.ts
//
// Feature: Run-of-show — Lot 2.
// POST : passe le segment de 'live' a 'done', set ended_at = now().
//
// Idempotent : si deja done → no-op 200. Si upcoming ou skipped → 409.
...
async function handler(
  ...
  if (segErr) {
    logger.error('[admin/events/seg/end] lookup error', segErr);
  ...
  if (segment.status === 'done') {
    return res.status(200).json({ segment, alreadyEnded: true });
  }
  if (segment.status !== 'live') {
    return res.status(409).json({
      error: `Le segment est en status '${segment.status}', impossible de le terminer.`,
  ...
  if (updErr) {
    logger.error('[admin/events/seg/end] update error', updErr);
    return res.status(500).json({ error: 'Failed to end segment.' });
  }
  if (!updated) {
    const { data: refreshed } = await admin
      ...
      .eq('tenant_id', ctx.tenantId)
      .maybeSingle();
    return res.status(200).json({ segment: refreshed, alreadyEnded: true });
  }
  ...
  }).catch((e) => logger.error('[admin/events/seg/end] outbox emit error', e));
  ...
  return res.status(200).json({ segment: updated, alreadyEnded: false });
}

export default withStaffRoute(
  withAdminIdempotency(handler, { key: 'admin-events-seg-end' }),
