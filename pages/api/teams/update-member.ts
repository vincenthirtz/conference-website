...
export default withSubjectRoute(
  async function handler(
    ...
    if (
      applyRateLimit(req, res, { max: 30, windowMs: 60_000 }, 'update-member')
    )
      return;

    // Sujet = l'appelant, ou la capitaine dépannée quand le staff agit à sa
    // place (`?as=…&act=1`). L'accès et l'équipe visée suivent le SUJET.
    ...
    // --- Audit ---------------------------------------------------------------
    // staff_id = la row staff de l'APPELANT — en act-as, `userId` est la
    // capitaine dépannée, qui n'a évidemment pas de row staff : journaliser sur
    // elle reviendrait à perdre la trace de qui a agi.
