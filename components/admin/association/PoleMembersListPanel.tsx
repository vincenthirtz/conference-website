// components/admin/association/PoleMembersListPanel.tsx
// Admin: membres des pôles de l'association, regroupés par pôle, + modale de
// création. Rendered as the "Pôles de l'asso" tab of the /admin/association hub.
//
// Endpoints:
//   GET    /api/admin/pole-members?includeInactive=&limit=200  → { items }
//   PATCH  /api/admin/pole-members/[id]   → { isActive }
//   DELETE /api/admin/pole-members/[id]
//
// Deep-links `?new=1` (ex-route /pole-members/new) ouvre la modale de création,
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-neutral-400 text-sm mt-1">
              {format(members.length > 1 ? t.summary_other : t.summary_one, {
                count: members.length,
                poles: POLE_KEYS.length,
              })}
            </p>
          </div>
              ...
              >
                <header className="flex items-center justify-between px-6 py-4 border-b border-neutral-700/50">
                  ...
                  <span className="text-xs text-neutral-400">
                    {format(
                      list.length > 1 ? t.memberCount_other : t.memberCount_one,
                      { count: list.length }
                    )}
                  </span>
                </header>
