...
//
// Page d'un article — le rendu, sans la source des données.
//
// Extraite de `pages/news/[slug].tsx` pour être partagée avec la route par
// espace. `basePath` porte le préfixe d'espace : sans lui, l'URL canonique et
// « À lire aussi » renverraient vers la plateforme depuis l'article d'un autre
// espace — le pire endroit pour perdre un lecteur venu d'un partage.
...
import type { NewsArticleProps } from '@/utils/publicData/newsArticle';
...
export default function NewsArticle({
  ...
  const canonical =
    slug && BASE_URL ? `${BASE_URL}${basePath}/news/${slug}` : undefined;
      ...
      <div className="mx-auto w-full max-w-[46rem] px-4 pt-24">
        ...
        {error ? (
          ...
          <article>
            ...
            {newsId && <Comments newsId={newsId} basePath={basePath} />}
          </article>
        )}
      </div>
    </div>
...
function Comments({
  ...
  // Les routes d'API ne portent PAS de préfixe d'espace : sous
  // `/mon-espace/news/...`, seul ce paramètre rattache l'appel au bon espace.
  // (Sur un domaine propre, l'hôte suffit — le paramètre est alors redondant
  // et sans effet.)
  const tenantParam = basePath ? `&tenant=${basePath.slice(1)}` : '';
  const t = useT(nsNewsDetail);
  const locale = useLocale();
  ...
  const loadComments = async () => {
    ...
    try {
      const res = await fetch(
        `/api/news/comments?newsId=${newsId}${tenantParam}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/news/comments${tenantParam ? `?${tenantParam.slice(1)}` : ''}`,
        {
          method: 'POST',
          headers: {
